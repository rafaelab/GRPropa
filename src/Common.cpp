...
double interpolate2d(double x, double y, const std::vector<double> &X, const std::vector<double> &Y, const std::vector<double> &Z) {
    ...
    /*double pf = 1 / ( (X[i+1] - X[i]) * (Y[i+1] - Y[i]) ); 
    double f1 = Q11 * (X[i+1] - x) * (Y[i+1] - y);
    double f2 = Q21 * (-X[i] + x) * (Y[i+1] - y);
    double f3 = Q12 * (X[i+1] - x) * (-Y[i] + y);
    double f4 = Q22 * (-X[i] + x) * (-Y[i] + y);
    
    double res = pf * (f1 + f2 + f3 + f4);*/
