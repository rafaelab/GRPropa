...
TextOutput::TextOutput(std::ostream &out, OutputType outputtype) 
    : Output(outputtype), out(&out) {
}

TextOutput::TextOutput(const std::string &filename) :  Output(), outfile(filename.c_str(), std::ios::binary), out(&outfile),  filename( filename) {
    if (kiss::ends_with(filename, ".gz"))
        gzip();
}

TextOutput::TextOutput(const std::string &filename, OutputType outputtype) : Output(outputtype), outfile(filename.c_str(), std::ios::binary), out(&outfile), filename(filename) {
    if (kiss::ends_with(filename, ".gz"))
        gzip();
}
...
void TextOutput::process(Candidate *c) const {
    ...
    if (fields.test(CurrentEnergyColumn))
        p += sprintf(buffer + p, "%.5e\t", c->current.getEnergy() / energyScale);
    if (fields.test(CurrentPositionColumn)) {
        if (oneDimensional) {
    ...
    if (fields.test(SourceEnergyColumn))
        p += sprintf(buffer + p, "%.5e\t", c->source.getEnergy() / energyScale);
    if (fields.test(SourcePositionColumn)) {
        if (oneDimensional) {
    ...
    if (fields.test(SourceDirectionColumn)) {
        if (not oneDimensional) {
            ...
            p += sprintf(buffer + p, "%.6e\t%.6e\t%.6e\t", pos.x, pos.y, pos.z);
        }
    }
    ...
    if (fields.test(CreatedEnergyColumn))
        p += sprintf(buffer + p, "%.5e\t", c->created.getEnergy() / energyScale);
    if (fields.test(CreatedPositionColumn)) {
        if (oneDimensional) {
...
#else
    throw std::runtime_error("GRPropa was build without Zlib compression!");
#endif
}
